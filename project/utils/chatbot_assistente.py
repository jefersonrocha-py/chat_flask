...
def show_error(message):
    """Exibe uma mensagem de erro amigável e registra o erro detalhado no log."""
    logging.error(f"Erro: {message}")
    return {"error": f"😕 Oops! Algo deu errado: {message}"}
...
def check_backend_connection():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise Exception(f"Erro ao conectar ao servidor backend: Status Code {response.status_code}")
    except requests.exceptions.Timeout:
        raise Exception(f"Timeout ao tentar conectar ao servidor backend em {OLLAMA_SERVER_URL}.")
    except Exception as e:
        raise Exception(f"Erro ao conectar ao servidor backend: {e}")

# Verificar conexão antes de prosseguir
backend_connected = check_backend_connection()

if not backend_connected:
    raise Exception("⚠️ O servidor backend não está disponível. Algumas funcionalidades podem estar limitadas.")
...
try:
    ...
    raise Exception("Erro crítico ao configurar embeddings ou chat model.")

# Criar diretório para armazenar arquivos
DOCUMENTS_DIR = os.path.join(os.getcwd(), "uploads", "files")
try:
    os.makedirs(DOCUMENTS_DIR, exist_ok=True)
except Exception as e:
    show_error(f"Erro ao criar diretório: {e}")
    raise Exception("Erro crítico ao criar diretório.")
...
def load_data(file_path):
    ...
    try:
        ...
        # Criar FAISS retriever com embeddings
...
def chatbot_assistente_logic(user_input, retriever=None):
