...
logging.basicConfig(
    filename="chatbot_coder_errors.log",
    level=logging.ERROR,
...
# CSS customizado para estilização (mantido igual ao original)
st.markdown(
    """
    <style>
...
def save_history():
    try:
        with open("chat_history_coder.json", "w") as f:
            json.dump({
...
def chatbot_coder_page():
    st.markdown(
        """
        <div class="chat-title"> FlowMind AI CodeMaster 💻</div>
        """,
        unsafe_allow_html=True
    )
    ...
    # Template do prompt ajustado para especialista em codificação
    conversation_prompt = ChatPromptTemplate.from_template(
        """
        Você é o CodeMaster AI, um assistente especializado em codificação. 
        Sua missão é ajudar usuários com dúvidas sobre programação, fornecer exemplos de código claros e bem comentados, 
        e oferecer soluções eficientes para problemas técnicos. 
        Use uma linguagem técnica, mas acessível, e inclua exemplos práticos sempre que possível.
        Se o usuário pedir algo fora de programação, gentilmente redirecione para o foco em código.
    ...
    # Entrada do usuário
    if user_input := st.chat_input("Digite sua dúvida ou pedido sobre código aqui:"):
    ...
    # Footer Dinâmico (mantido igual ao original)
    st.markdown("""
        <div class="custom-footer">
            <div class="footer-content">
                ...
                <div class="footer-center">
                    FlowMind AI CodeMaster © 2025 - Todos os direitos reservados
                </div>
